...
if inspector.has_table('users') == False:
    #create the table
    create_users_table()
    #ins = Users.insert()
    #ins = Users.insert().values(id=1,
    #                      username ='pythonuser',
    #                      email='pythonuser@pythonuser.com',
    #                      password='pythonuser'  )
    #conn = engine.connect()
...
home = html.Div([dcc.Location(id='home', refresh=True)
        , html.Br()
        , html.Div(html.H2('Welcome - Please login'))
        , html.Br()
        , html.Div([login])
        , html.Button(id='back-button', children='Go back', n_clicks=0)
        ])
